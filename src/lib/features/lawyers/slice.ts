...
import { Lawyer, LawyerDto, LawyerQuery } from '@/types/lawyer'
...
const lawyerSlice = createSlice({
   ...
   extraReducers: (builder) => {
      builder
         ...
         .addCase(fetchLawyers.fulfilled, (state, action: PayloadAction<any>) => { // Type as any or specific response type
            state.loading = false
            state.lawyers = action.payload.data ? action.payload.data : action.payload // Adapt based on API response structure
         ...
         .addCase(fetchLawyer.fulfilled, (state, action) => {
            state.loading = false
            state.selectedLawyer = action.payload // Assuming fetchLawyer returns just the lawyer object
         })
         ...
         .addCase(createLawyer.fulfilled, (state, action) => {
            ...
            state.lawyers = [action.payload, ...state.lawyers]
            state.total += 1
         })
         ...
         .addCase(updateLawyer.fulfilled, (state, action) => {
            ...
            const index = state.lawyers.findIndex((l) => l.id === action.payload.id)
            if (index !== -1) {
